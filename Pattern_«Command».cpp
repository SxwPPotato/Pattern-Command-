...
private:
    ...
    std::ofstream out;
public:
    message_to_the_file(Message& m, std::string file_name_) : mess(m), file_name(file_name_){ out.open(file_name,std::ios::app); }
    void execute()
    {
        if (out.is_open())
        {     
...
int main()
{
    ...
    message_to_the_file TV_on_file(tv, "file");
    message_to_the_console Lights_on_console(lights);
    message_to_the_file Lights_on_file(lights, "file");
